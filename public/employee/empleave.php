...
<html lang="en">
  <head>
    ...
    <title>Leave Application | Leave Manager</title>
    ...
    <style>
      ...
      * {
      ...
      /* Leave form and history sections */
      .leave-form-section,
      ...
      .leave-form-section::before,
      .leave-history-section::before {
        content: "";
        position: absolute;
        top: -50%;
        left: -50%;
        ...
        <section class="leave-form-section">
          ...
          <form id="leaveForm">
            <div class="form-group">
              <label for="leaveTitle" class="form-label">Leave Title</label>
              <input
                type="text"
                id="leaveTitle"
                class="form-input"
          ...
          <div class="leave-list" id="leaveList">
            <div class="leave-item">
              <div class="leave-header">
                <span class="leave-title">Annual Vacation</span>
                <span class="leave-status status-approved">
              ...
              <div class="leave-reason">Family vacation to the beach</div>
            </div>

            <div class="leave-item">
              <div class="leave-header">
                <span class="leave-title">Medical Leave</span>
              ...
              <div class="leave-reason">Doctor's appointment and recovery</div>
            </div>

            <div class="leave-item">
              <div class="leave-header">
                <span class="leave-title">Emergency Leave</span>
                <span class="leave-status status-rejected">
              ...
              <div class="leave-reason">
                Family emergency (insufficient notice)
              </div>
            </div>
          </div>
        </section>
   ...
   * User Initials
   ----------------------------- */
      ...
      async function loadUserInitials() {
        try {
          const res = await fetch("initails.php", { credentials: "include" });
          if (!res.ok) throw new Error("Network error");
          const data = await res.json();

          const initials =
            data.success && data.name ? getInitials(data.name) : "NA";
          document.getElementById("profileInitials").textContent = initials;
        } catch (err) {
          console.error("Error loading initials:", err);
   ...
   * Leave History
   ----------------------------- */
      async function loadLeaveHistory() {
        try {
          const res = await fetch("get_leaves.php", { credentials: "include" });
          if (!res.ok) throw new Error("Network error");
          ...
          data.success
            ...
            : showToast(
                data.message || "Failed to load leave history",
                "error"
              );
        } catch (err) {
          showToast("Error loading leave history", "error");
      ...
      <div class="leave-item">
        <div class="leave-header">
          <span class="leave-title">${l.title}</span>
          <span class="leave-status status-${l.status}">
            <i class="fas ${
              l.status === "approved"
                ? "fa-check-circle"
                : l.status === "rejected"
   ...
   ----------------------------- */
      document.addEventListener("DOMContentLoaded", async () => {
        ...
        leaveForm.addEventListener("submit", async (e) => {
          e.preventDefault();
          const applyBtn = leaveForm.querySelector(".apply-button");

          applyBtn.disabled = true;
          spinner.style.display = "inline-block";

          const payload = {
            title: leaveForm.leaveTitle.value,
            leave_type: leaveForm.leaveType.value,
            reason: leaveForm.leaveReason.value,
            start_date: leaveForm.startDate.value,
            end_date: leaveForm.endDate.value,
          };

          try {
            const res = await fetch("apply_leave.php", {
              method: "POST",
            ...
            if (result.success) {
              ...
              showToast(result.message || "Failed to submit leave", "error");
            }
          } catch (err) {
        ...
        leaveForm.startDate.min = today;
        leaveForm.endDate.min = today;
        leaveForm.startDate.addEventListener("change", () => {
          leaveForm.endDate.min = leaveForm.startDate.value;
        });
      });
    </script>
  </body>
